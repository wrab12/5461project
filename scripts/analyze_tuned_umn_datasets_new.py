...
def analyze_tuned_dataset(h5ad_path, level):
    ...
    print(f"    {'Method':<12} {'ARI':<10} {'NMI':<10} {'Silhouette':<12} {'N_Clusters':<12}")
    print("    " + "-"*58)
...
def main():
    ...
    print(f"\n{'Level':<10} {'Best Method':<15} {'ARI':<10} {'NMI':<10} {'Silhouette':<12}")
    print("-"*65)
    ...
    for level, results in all_results.items():
        ...
        print(f"{level:<10} {best_method.capitalize():<15} {ari:<10.4f} {nmi:<10.4f} {sil:<12.4f}")
    ...
    results_df.to_csv("tuned_umn_clustering_evaluation_results.csv", index=False)
    print("\nSaved evaluation results to: tuned_umn_clustering_evaluation_results.csv")
